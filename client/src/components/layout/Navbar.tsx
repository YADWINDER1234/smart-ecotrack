...
import { Sun, Moon, Sprout, Scan, LogOut, LayoutDashboard, UserPlus, LogIn } from "lucide-react";
    ...
    <div className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60">
      <div className="container mx-auto flex h-16 items-center justify-between px-4 md:px-8">
        <Link to="/" className="flex items-center space-x-2 mr-6">
          ...
          <span className="hidden font-bold sm:inline-block">Smart EcoTrack</span>
        </Link>
        ...
        <nav className="flex flex-1 items-center space-x-6 text-sm font-medium">
          ...
          {user?.role === "ADMIN" && (
            <>
              <Link to="/admin/products" className="transition-colors hover:text-primary text-muted-foreground">
                Admin
              </Link>
              <Link to="/admin/complaints" className="transition-colors hover:text-primary text-muted-foreground flex items-center gap-1">
                Complaints
                {openCount > 0 && (
          ...
          {user?.role === "RECYCLER" && (
            <Link to="/recycler/events" className="transition-colors hover:text-primary text-muted-foreground">
              Recycler
            </Link>
          )}
        </nav>
